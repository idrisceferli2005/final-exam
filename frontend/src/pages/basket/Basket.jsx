          ...
          {basket && basket.length > 0 ? (
            basket.map((item) => (
              <tr key={item._id}>
                ...
                <td className="d-flex gap-3" style={{ cursor: "pointer" }}>
                  <button className="btn btn-danger" onClick={() => dispatch(plusBtn(item))}>+</button>
                  <p>{item.count}</p>
                  <button className="btn btn-danger" onClick={() => dispatch(minusBtn(item))}>-</button>
